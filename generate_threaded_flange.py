"""
Threaded Flange Generator (RF and RTJ)
Generates ASME B16.5 threaded flanges with raised face or ring joint facing.Units: MM (metric flange standards)
Threaded Features:
- Internal NPT (National Pipe Thread - Tapered) threads per ASME B1.20.1
- Threads not modeled geometrically (too complex) - simple bore shown
- Available: All classes 150-2500, size ranges vary by class
"""
...
from threaded_flange_data import get_threaded_dimensions
...
def make_threaded_hub(thread_bore, hub_outer_diameter, hub_length, z_flange_bottom):
    """
    Create threaded hub - simple straight cylinder (threads not modeled).
    
    Args:
        thread_bore: NPT thread bore diameter
    ...
    Returns:
        Hub shape
    """
    thread_bore_radius = thread_bore / 2.0
    ...
    # Cut inner bore (NPT threads not modeled - just straight bore)
    hub_bore_cyl = make_cylinder(thread_bore_radius, hub_length, z_hub_bottom)
...
def generate_threaded_flange(nps, pressure_class, facing='RF', output_dir=None):
    """
    Generate a threaded flange.
    ...
    """
    print(f"\n=== Generating Threaded Flange: NPS {nps}, Class {pressure_class}, Facing: {facing} ===")
    ...
    # Get threaded bore dimensions
    threaded_dims = get_threaded_dimensions(nps, pressure_class)
    thread_bore = threaded_dims['thread_bore']
    thread_size = threaded_dims['thread_size']
    ...
    hub_diameter = flange_dims.get('hub_base_diameter', thread_bore * 1.4)
    ...
    print(f"Thread size: {thread_size} NPT")
    print(f"Thread bore: {thread_bore:.1f}mm")
    print(f"Hub OD: {hub_diameter:.1f}mm")
    print(f"Hub length: {hub_length:.1f}mm (estimated)")
    print(f"Note: NPT threads not modeled geometrically")
    ...
    through_bore = make_cylinder(thread_bore_radius, bore_height, z_flange_bottom)
    body = BRepAlgoAPI_Cut(body, through_bore).Shape()
    
    # ===== Layer 3: Threaded hub =====
    print(f"Layer 3: Threaded hub (bore={thread_bore:.1f}mm, OD={hub_diameter:.1f}mm)")
    threaded_hub = make_threaded_hub(thread_bore, hub_diameter, hub_length, z_flange_bottom)
    body = BRepAlgoAPI_Fuse(body, threaded_hub).Shape()
    ...
    if output_dir is None:
        output_dir = r'C:\Users\Jeff\EquationParadise\CAD\Threaded_Flanges'
    
    os.makedirs(output_dir, exist_ok=True)
    
    facing_str = 'RTJ' if facing == 'RTJ' else 'RF'
    filename = f"THD-{nps.replace('/', '-')}-Class{pressure_class}-{facing_str}.step"
...
if __name__ == '__main__':
    if len(sys.argv) >= 3:
        ...
        generate_threaded_flange(nps, pressure_class, facing)
    else:
        print("Usage: python generate_threaded_flange.py <NPS> <Class> [RF|RTJ]")
        print("Examples:")
        print("  python generate_threaded_flange.py 2 150 RF")
        print("  python generate_threaded_flange.py 1-1/2 600 RTJ")
        print("  python generate_threaded_flange.py 3 300")
